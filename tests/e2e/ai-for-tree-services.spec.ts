...
// Ticket 0092 - AI-for-tree-services long-tail landing page funneling into the
// existing home-services demos (lead-responder, estimate, voice-followup).
// Each test maps 1:1 to an acceptance-criteria box on the ticket.
// Structurally modeled on tests/e2e/ai-for-garage-door-companies.spec.ts
// (ticket 0089, the freshest peer in the home-services trade-page family)
...
// /ai-for-tree-services-scoped Service, BreadcrumbList, or FAQPage
...
// lesson, `gotoTreeServices` waits for the RouteFallback spinner to
...
async function gotoTreeServices(page: Page): Promise<void> {
  const response = await page.goto('/ai-for-tree-services', {
    waitUntil: 'domcontentloaded',
  });
  expect(response, 'no response for /ai-for-tree-services').not.toBeNull();
  // Box 10-1: GET /ai-for-tree-services returns 200.
  expect(
    response!.status(),
    `/ai-for-tree-services returned ${response!.status()}`,
  ).toBeLessThan(400);
...
// Box 10-2: page renders an H1 containing "Tree Services"
// (case-insensitive). Also covers the ticket's "hero H1 'AI for Tree
// Services'" acceptance box.
test('renders H1 naming Tree Services and a defensible subtitle', async ({
  page,
}) => {
  await gotoTreeServices(page);

  const h1 = page.getByRole('heading', { level: 1 });
  await expect(h1).toBeVisible();
  const h1Text = ((await h1.textContent()) ?? '').toLowerCase();
  expect(h1Text).toMatch(/tree services/);
  ...
  await expect
    ...
    .toEqual(expect.arrayContaining([expect.stringMatching(/tree service/i)]));

  // Subtitle acceptance: the page body names the pain of after-hours
  // storm-damage inbounds going to voicemail, long-tail species-and-access
  // triage, and next-day appointment-window follow-ups.
  const body = (await page.locator('body').innerText()).toLowerCase();
  expect(body).toMatch(/voicemail|storm|limb|after.?hours|emergency/);
  expect(body).toMatch(/species|oak|pine|maple|canopy|trunk|drop.?zone|bucket|chipper/);
  expect(body).toMatch(/appointment|technician|window|follow.?up|arrival|crew/);
});
...
test('surfaces three demo CTA links pointing at the existing home-services demos', async ({
  ...
  // Each demo-card CTA carries data-testid="tree-services-demo-cta" so
  // the spec can find the exact three cards without depending on body copy.
  const ctas = page.locator('[data-testid="tree-services-demo-cta"]');
...
// "Tree Services". Per the 2026-05-30 second-@type lesson, this predicate
// is URL-scoped (gotoTreeServices was called first) so it cannot collide
// with any predecessor trade-page's exactly-one-Service assertion.
test('emits exactly one Service JSON-LD block naming Tree Services', async ({
  page,
}) => {
  await gotoTreeServices(page);
  ...
  expect(svc.name ?? '').toMatch(/tree services/i);
  expect(svc.url).toBe('https://digitalcraftai.com/ai-for-tree-services');
...
// entries whose names are "Home" and "AI for Tree Services". URL-scoped
// per the 2026-05-30 lesson.
test('emits exactly one BreadcrumbList block whose second item is AI for Tree Services', async ({
  page,
}) => {
  await gotoTreeServices(page);
  const blocks = await readJsonLdBlocks(page);
  ...
  expect(items[1].name ?? '').toBe('AI for Tree Services');
  expect(items[1].item).toBe('https://digitalcraftai.com/ai-for-tree-services');
...
test('emits exactly one FAQPage block mirroring the visible FAQ cards', async ({
  ...
  const visibleCards = page.locator('[data-testid="tree-services-faq-card"]');
...
// html.dark class the way the ticket 0089 spec does and re-check that the
...
test('renders in light and dark mode with the same demo-card CTAs visible', async ({
  ...
  const lightCtas = page.locator('[data-testid="tree-services-demo-cta"]');
  await expect(lightCtas).toHaveCount(3);

  await page.evaluate(() => document.documentElement.classList.add('dark'));
  await expect(page.getByRole('heading', { level: 1 })).toBeVisible();
  const darkCtas = page.locator('[data-testid="tree-services-demo-cta"]');
  ...
  await expect(page.getByRole('heading', { level: 1 })).toBeVisible();
  await expect(
    page.locator('[data-testid="tree-services-demo-cta"]'),
  ).toHaveCount(3);
});
...
test('bottom strategy-call CTA fires the tree_services_book_call analytics event', async ({
  page,
}) => {
  await gotoTreeServices(page);
  ...
  const bottomCta = page
    .locator('[data-testid="tree-services-footer-cta"]')
    .first();
  ...
  expect(
    labels.some((l) =>
      /tree_services_book_call.*tree_services_footer/.test(l),
    ),
    `expected an event_label like "tree_services_book_call - tree_services_footer", got: ${labels.join(' | ')}`,
  ).toBe(true);
});

// Standard Box 11: no first-party /api/ call fires when rendering the page,
// and the /ai-for-tree-services URL appears in the generated sitemap
...
test('makes no first-party /api/ call and appears in sitemap.xml', async ({
  ...
  expect(
    apiCalls,
    `the tree-services landing page should make no first-party /api/ call:\n${apiCalls.join('\n')}`,
  ).toEqual([]);
  ...
  expect(xml).toContain('https://digitalcraftai.com/ai-for-tree-services');

  // The route is in the ROUTES allow-list.
  expect(STATIC_ROUTES.has('/ai-for-tree-services')).toBe(true);
});
