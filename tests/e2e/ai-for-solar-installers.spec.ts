...
// Ticket 0084 - AI-for-solar-installers long-tail landing page funneling into
...
// modeled on tests/e2e/ai-for-moving-companies.spec.ts (ticket 0080, the
...
// /ai-for-solar-installers-scoped Service, BreadcrumbList, or FAQPage block
...
// lesson, `gotoSolarInstallers` waits for the RouteFallback spinner to
...
async function gotoSolarInstallers(page: Page): Promise<void> {
  const response = await page.goto('/ai-for-solar-installers', {
    waitUntil: 'domcontentloaded',
  });
  expect(response, 'no response for /ai-for-solar-installers').not.toBeNull();
  // Box 9-1: GET /ai-for-solar-installers returns 200.
  expect(
    response!.status(),
    `/ai-for-solar-installers returned ${response!.status()}`,
  ).toBeLessThan(400);
...
// Box 9-2: page renders an H1 containing "Solar Installers"
// (case-insensitive). Also covers the ticket's "hero H1 'AI for Solar
// Installers'" acceptance box.
test('renders H1 naming Solar Installers and a defensible subtitle', async ({
  page,
}) => {
  await gotoSolarInstallers(page);

  const h1 = page.getByRole('heading', { level: 1 });
  await expect(h1).toBeVisible();
  const h1Text = ((await h1.textContent()) ?? '').toLowerCase();
  expect(h1Text).toMatch(/solar installers/);
  ...
  await expect
    ...
    .toEqual(expect.arrayContaining([expect.stringMatching(/solar/i)]));

  // Subtitle acceptance: the page body names the pain of walk-in rooftop
  // assessment inbounds going to voicemail during a site visit.
  const body = (await page.locator('body').innerText()).toLowerCase();
  expect(body).toMatch(/voicemail|rooftop|assessment|site visit/);
  expect(body).toMatch(/system.?siz|financing|utility|interconnection|battery/);
  expect(body).toMatch(/permit|inspection|grid|follow.?up/);
});
...
test('surfaces three demo CTA links pointing at the existing home-services demos', async ({
  ...
  // Each demo-card CTA carries data-testid="solar-installers-demo-cta" so the
  // spec can find the exact three cards without depending on body copy.
  const ctas = page.locator('[data-testid="solar-installers-demo-cta"]');
...
// "Solar Installers". Per the 2026-05-30 second-@type lesson, this predicate
// is URL-scoped (gotoSolarInstallers was called first) so it cannot collide
// with any predecessor trade-page's exactly-one-Service assertion.
test('emits exactly one Service JSON-LD block naming Solar Installers', async ({
  page,
}) => {
  await gotoSolarInstallers(page);
  ...
  expect(svc.name ?? '').toMatch(/solar installers/i);
  expect(svc.url).toBe('https://digitalcraftai.com/ai-for-solar-installers');
...
// entries whose names are "Home" and "AI for Solar Installers". URL-scoped
// per the 2026-05-30 lesson.
test('emits exactly one BreadcrumbList block whose second item is AI for Solar Installers', async ({
  page,
}) => {
  await gotoSolarInstallers(page);
  const blocks = await readJsonLdBlocks(page);
  ...
  expect(items[1].name ?? '').toBe('AI for Solar Installers');
  expect(items[1].item).toBe(
    'https://digitalcraftai.com/ai-for-solar-installers',
  );
...
// html.dark class the way the ticket 0080 spec does and re-check that the
...
test('renders in light and dark mode with the same demo-card CTAs visible', async ({
  ...
  const lightCtas = page.locator('[data-testid="solar-installers-demo-cta"]');
  await expect(lightCtas).toHaveCount(3);

  await page.evaluate(() => document.documentElement.classList.add('dark'));
  await expect(page.getByRole('heading', { level: 1 })).toBeVisible();
  const darkCtas = page.locator('[data-testid="solar-installers-demo-cta"]');
  ...
  await expect(page.getByRole('heading', { level: 1 })).toBeVisible();
  await expect(
    page.locator('[data-testid="solar-installers-demo-cta"]'),
  ).toHaveCount(3);
});
...
test('bottom strategy-call CTA fires the solar_installers_book_call analytics event', async ({
  page,
}) => {
  await gotoSolarInstallers(page);
  ...
  const bottomCta = page
    .locator('[data-testid="solar-installers-footer-cta"]')
    .first();
  ...
  expect(
    labels.some((l) =>
      /solar_installers_book_call.*solar_installers_footer/.test(l),
    ),
    `expected an event_label like "solar_installers_book_call - solar_installers_footer", got: ${labels.join(' | ')}`,
  ).toBe(true);
});

// Standard Box 10: no first-party /api/ call fires when rendering the page,
// and the /ai-for-solar-installers URL appears in the generated sitemap
...
test('makes no first-party /api/ call and appears in sitemap.xml', async ({
  ...
  expect(
    apiCalls,
    `the solar-installers landing page should make no first-party /api/ call:\n${apiCalls.join('\n')}`,
  ).toEqual([]);
  ...
  expect(xml).toContain('https://digitalcraftai.com/ai-for-solar-installers');

  // The route is in the ROUTES allow-list.
  expect(STATIC_ROUTES.has('/ai-for-solar-installers')).toBe(true);
});
