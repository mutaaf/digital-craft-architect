...
// Ticket 0056 - AI-for-pest-control long-tail landing page funneling into the
...
// directly on tests/e2e/ai-for-cleaning-services.spec.ts (ticket 0050, the
// closest peer because both are residential service trades with a recurring-
// service revenue model and an after-hours emergency funnel). NOT modeled on
// the property-managers spec because that page routes into /realestate/demo/*
// (the wrong family for pest control).
//
// IMPORTANT: /ai-for-pest-control is NOT in the index.html SEO Pilot `pages`
...
// /ai-for-pest-control-scoped BreadcrumbList block cannot collide.
// demo-breadcrumbs.spec.ts iterates a hard-coded STARTER_ROUTES list that
// does NOT include /ai-for-pest-control, so it is unaffected.
...
async function gotoPestControl(page: Page): Promise<string[]> {
  const errors: string[] = [];
  page.on('pageerror', (e) => {
    const msg = `pageerror: ${e.message}`;
    if (!isIgnorable(msg)) errors.push(msg);
  });
  const response = await page.goto('/ai-for-pest-control', { waitUntil: 'domcontentloaded' });
  expect(response, 'no response for /ai-for-pest-control').not.toBeNull();
  expect(
    response!.status(),
    `/ai-for-pest-control returned ${response!.status()}`,
  ).toBeLessThan(400);
  await expect
    .poll(
...
// Box 1: a new /ai-for-pest-control route renders a page whose Helmet-emitted
// description and visible H1 explicitly name "pest control," and whose body
// lists three trade-specific pain themes (after-hours rodent/wasp emergency
// calls hitting voicemail, quarterly recurring-service renewal reminders
// slipping off the office manager's list, review windows closing before
// anyone asks after a finished treatment) with defensible language and no
// invented client names, dollar figures, or efficacy percentages.
test('renders pest-control H1, Helmet meta description, and three pain points', async ({
  page,
}) => {
  const errors = await gotoPestControl(page);
  ...
  // H1: the visible page heading must name the trade. The substring "Pest
  // Control" (case-insensitive) covers both "Pest Control Companies" and
  // "Pest Control Business" copy variants.
  const h1 = page.getByRole('heading', { level: 1 });
  await expect(h1).toBeVisible();
  const h1Text = ((await h1.textContent()) ?? '').toLowerCase();
  expect(h1Text).toMatch(/pest control/);
  ...
  await expect
    ...
    .toEqual(expect.arrayContaining([expect.stringMatching(/pest control/i)]));

  const descriptions = await page
    .locator('head meta[name="description"]')
    .evaluateAll((nodes) => nodes.map((n) => n.getAttribute('content') ?? ''));
  const helmetDescription = descriptions[descriptions.length - 1];
  expect(helmetDescription.toLowerCase()).toMatch(/pest control/);
  expect(helmetDescription.length).toBeGreaterThan(40);

  // Three trade-specific pain point themes named on the page.
  const body = (await page.locator('body').innerText()).toLowerCase();
  expect(body).toMatch(/after[-\s]?hours|rodent|wasp|voicemail|emergency/);
  expect(body).toMatch(/recurring|renewal|quarterly/);
...
test('surfaces three demo CTA links pointing at the existing home-services demos', async ({
  ...
  // data-testid="pestcontrol-demo-cta" so the spec can locate them without
  ...
  const ctas = page.locator('[data-testid="pestcontrol-demo-cta"]');
...
// (Home -> AI for Pest Control) that parses as valid JSON. Per the
// 2026-05-30 second-@type lesson, the predicate is per-URL scoped
// (gotoPestControl was called first), so this toHaveLength(1) cannot
// collide with a sibling page's BreadcrumbList block. The seven existing
// trade pages emit BreadcrumbList only - the eighth follows that convention.
test('emits BreadcrumbList JSON-LD block that parses and names AI for Pest Control', async ({
  page,
}) => {
  const errors = await gotoPestControl(page);
  ...
  // Second ListItem must explicitly name "AI for Pest Control" per ticket
  // acceptance box 2.
  expect(items[1].name ?? '').toBe('AI for Pest Control');
  expect(items[1].item).toBe('https://digitalcraftai.com/ai-for-pest-control');
...
// title/description/canonical/OG, and the /ai-for-pest-control URL appears
...
test('renders Navbar + Footer, has Helmet canonical, and appears in sitemap.xml', async ({
  ...
  await expect
    ...
    .toEqual(
      expect.arrayContaining([expect.stringContaining('/ai-for-pest-control')]),
    );
  ...
  await expect
    ...
    .toEqual(
      expect.arrayContaining([expect.stringMatching(/pest control/i)]),
    );
  ...
  expect(xml).toContain('https://digitalcraftai.com/ai-for-pest-control');

  // The new route is in the ROUTES allow-list (canonical src/data/routes.ts
  // per the 2026-06-07 lesson) - the spec re-exports it.
  expect(STATIC_ROUTES.has('/ai-for-pest-control')).toBe(true);
...
// fired on each demo CTA with a location label that includes "pestcontrol"
// so the page's funnel is measurable in GA independently of the seven other
// trade pages.
test('renders in light/dark on mobile, no em-dash, fires trackCTAClick with pestcontrol label', async ({
  page,
}) => {
  const errors = await gotoPestControl(page);
  ...
  // demo CTA click pushes an event with a pestcontrol location label.
  await page.evaluate(() => {
  ...
  expect(
    labels.some((l) => /pestcontrol/i.test(l)),
    `expected at least one cta_click event_label to name pestcontrol, got: ${labels.join(' | ')}`,
  ).toBe(true);
...
test('makes no first-party /api/ call when rendering and no new hostname appears', async ({
  ...
  expect(
    apiCalls,
    `the pest-control landing page should make no first-party /api/ call:\n${apiCalls.join('\n')}`,
  ).toEqual([]);
  expect(errors).toEqual([]);
});
