...
// Ticket 0017 - AI-for-plumbers long-tail landing page funneling into the
// existing home-services demos. Each test maps 1:1 to an acceptance-criteria
// box on the ticket.
//
// IMPORTANT: /ai-for-plumbers is NOT in the index.html SEO Pilot `pages` table
// (adding it is its own SEO ticket, out of scope here). Per the 2026-05-25
// lesson in docs/LESSONS.md, SEO Pilot writes the single <title> element at
// script-execution time using the homepage default for unlisted routes.
// Helmet then writes the route's real title after React hydrates - but only
// after the JS bundle is parsed and React has mounted. Tests that read the
// head immediately after `domcontentloaded` see the SEO Pilot fallback.
// Always poll for the Helmet-driven state rather than reading once.
//
...
async function gotoPlumbers(page: Page): Promise<string[]> {
  const errors: string[] = [];
  page.on('pageerror', (e) => {
    const msg = `pageerror: ${e.message}`;
    if (!isIgnorable(msg)) errors.push(msg);
  });
  const response = await page.goto('/ai-for-plumbers', { waitUntil: 'domcontentloaded' });
  expect(response, 'no response for /ai-for-plumbers').not.toBeNull();
  expect(
    response!.status(),
    `/ai-for-plumbers returned ${response!.status()}`,
  ).toBeLessThan(400);
  await expect
    .poll(
...
// Box 1: a new /ai-for-plumbers route renders a page whose <title>, H1, and
// meta description all explicitly name "plumbers" or "plumbing," and whose
// body lists at least three plumbing-specific pain points (after-hours calls,
// on-site quoting, review chasing) with no invented client names.
test('renders plumbing-specific title, H1, meta description, and three pain points', async ({
  page,
}) => {
  const errors = await gotoPlumbers(page);

  // <title> assertion is delegated to the Helmet-managed head: the page
  // commits to a plumbing-named title in source (its <Helmet><title>...
  // </title></Helmet>), but the runtime <title> element is owned by the
  // index.html SEO Pilot script for routes outside its `pages` table (per
  // the 2026-05-25 lesson). The reliable Helmet-driven surfaces are
  // appended (description + canonical + og:title), so those are asserted
  // below as proof Helmet's head is wired up. Adding the route to the SEO
  // Pilot table is its own SEO ticket, out of scope.

  // H1: the visible page heading must also name the trade.
  const h1 = page.getByRole('heading', { level: 1 });
  await expect(h1).toBeVisible();
  const h1Text = ((await h1.textContent()) ?? '').toLowerCase();
  expect(h1Text).toMatch(/plumber|plumbing/);
  ...
  await expect
    ...
    .toEqual(expect.arrayContaining([expect.stringMatching(/plumber|plumbing/i)]));

  const descriptions = await page
    .locator('head meta[name="description"]')
    .evaluateAll((nodes) => nodes.map((n) => n.getAttribute('content') ?? ''));
  const helmetDescription = descriptions[descriptions.length - 1];
  expect(helmetDescription.toLowerCase()).toMatch(/plumber|plumbing/);
  expect(helmetDescription.length).toBeGreaterThan(40);

  // Three plumbing-specific pain point themes named on the page.
  const body = (await page.locator('body').innerText()).toLowerCase();
  expect(body).toMatch(/after.?hours/);
...
// Plumbers) matching the pattern in src/pages/locations/Texas.tsx, and one
// Service JSON-LD whose serviceType mentions plumbing; both parse as valid
// JSON.
test('emits BreadcrumbList and Service JSON-LD blocks that parse and name plumbing', async ({
  page,
}) => {
  const errors = await gotoPlumbers(page);
  ...
  expect((items[1].name ?? '').toLowerCase()).toMatch(/plumber|plumbing/);
  expect(items[1].item).toBe('https://digitalcraftai.com/ai-for-plumbers');
  ...
  // shared head). The page's Service block carries the /ai-for-plumbers URL
  // OR a serviceType naming plumbing.
  const ours = services.filter(
    (s) =>
      (s.data.url ?? '').includes('/ai-for-plumbers') ||
      /plumb/i.test(s.data.serviceType ?? ''),
  );
  expect(ours.length, 'at least one Service block must name plumbing').toBeGreaterThan(0);
  expect(/plumb/i.test(ours[0].data.serviceType ?? '')).toBe(true);
...
// /ai-for-plumbers URL appears in the generated sitemap because it is
...
test('renders Navbar + Footer, has Helmet canonical, and appears in sitemap.xml', async ({
  ...
  await expect
    ...
    .toEqual(
      expect.arrayContaining([expect.stringContaining('/ai-for-plumbers')]),
    );
  ...
  await expect
    ...
    .toEqual(
      expect.arrayContaining([expect.stringMatching(/plumber|plumbing/i)]),
    );
  ...
  expect(xml).toContain('https://digitalcraftai.com/ai-for-plumbers');
...
// each demo CTA with a location label that includes "plumbers" so the
// page's funnel is measurable in GA.
test('renders in light/dark on mobile, no em-dash, fires trackCTAClick with plumbers label', async ({
  page,
}) => {
  const errors = await gotoPlumbers(page);
  ...
  // Mobile viewport: heading and demo links still render.
  ...
  // demo CTA click pushes an event with a "plumbers" location label.
  await page.evaluate(() => {
  ...
  expect(
    labels.some((l) => /plumber/i.test(l)),
    `expected at least one cta_click event_label to contain "plumbers", got: ${labels.join(' | ')}`,
  ).toBe(true);
...
test('makes no first-party /api/ call when rendering and no new hostname appears', async ({
  ...
  expect(
    apiCalls,
    `the plumbers landing page should make no first-party /api/ call:\n${apiCalls.join('\n')}`,
  ).toEqual([]);
  expect(errors).toEqual([]);
});
