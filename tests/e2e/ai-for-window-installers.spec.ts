...
// Ticket 0087 - AI-for-window-installers long-tail landing page funneling into
...
// modeled on tests/e2e/ai-for-solar-installers.spec.ts (ticket 0084, the
// freshest peer in the home-services trade-page family) with the standard
// coverage the trade-page family demands: a Service block, a
...
// /ai-for-window-installers-scoped Service, BreadcrumbList, or FAQPage
...
// lesson, `gotoWindowInstallers` waits for the RouteFallback spinner to
...
async function gotoWindowInstallers(page: Page): Promise<void> {
  const response = await page.goto('/ai-for-window-installers', {
    waitUntil: 'domcontentloaded',
  });
  expect(response, 'no response for /ai-for-window-installers').not.toBeNull();
  // Box 10-1: GET /ai-for-window-installers returns 200.
  expect(
    response!.status(),
    `/ai-for-window-installers returned ${response!.status()}`,
  ).toBeLessThan(400);
...
// Box 10-2: page renders an H1 containing "Window Installers"
// (case-insensitive). Also covers the ticket's "hero H1 'AI for Window
// Installers'" acceptance box.
test('renders H1 naming Window Installers and a defensible subtitle', async ({
  page,
}) => {
  await gotoWindowInstallers(page);

  const h1 = page.getByRole('heading', { level: 1 });
  await expect(h1).toBeVisible();
  const h1Text = ((await h1.textContent()) ?? '').toLowerCase();
  expect(h1Text).toMatch(/window installers/);
  ...
  await expect
    ...
    .toEqual(expect.arrayContaining([expect.stringMatching(/window/i)]));

  // Subtitle acceptance: the page body names the pain of whole-house
  // replacement inbounds going to voicemail during an in-home measurement,
  // the frame-material / glass-package triage gap, and the manufacturer
  // lead-time / permit follow-up gap.
  const body = (await page.locator('body').innerText()).toLowerCase();
  expect(body).toMatch(/voicemail|measurement|whole.?house|in.?home/);
  expect(body).toMatch(/frame|glass|vinyl|fiberglass|argon|glazing/);
  expect(body).toMatch(/permit|manufacturer|lead.?time|install/);
});
...
test('surfaces three demo CTA links pointing at the existing home-services demos', async ({
  ...
  // Each demo-card CTA carries data-testid="window-installers-demo-cta" so
  // the spec can find the exact three cards without depending on body copy.
  const ctas = page.locator('[data-testid="window-installers-demo-cta"]');
...
// "Window Installers". Per the 2026-05-30 second-@type lesson, this predicate
// is URL-scoped (gotoWindowInstallers was called first) so it cannot collide
// with any predecessor trade-page's exactly-one-Service assertion.
test('emits exactly one Service JSON-LD block naming Window Installers', async ({
  page,
}) => {
  await gotoWindowInstallers(page);
  ...
  expect(svc.name ?? '').toMatch(/window installers/i);
  expect(svc.url).toBe('https://digitalcraftai.com/ai-for-window-installers');
...
// entries whose names are "Home" and "AI for Window Installers". URL-scoped
// per the 2026-05-30 lesson.
test('emits exactly one BreadcrumbList block whose second item is AI for Window Installers', async ({
  page,
}) => {
  await gotoWindowInstallers(page);
  const blocks = await readJsonLdBlocks(page);
  ...
  expect(items[1].name ?? '').toBe('AI for Window Installers');
  expect(items[1].item).toBe(
    'https://digitalcraftai.com/ai-for-window-installers',
  );
...
// html.dark class the way the ticket 0084 spec does and re-check that the
...
test('renders in light and dark mode with the same demo-card CTAs visible', async ({
  ...
  const lightCtas = page.locator('[data-testid="window-installers-demo-cta"]');
  await expect(lightCtas).toHaveCount(3);

  await page.evaluate(() => document.documentElement.classList.add('dark'));
  await expect(page.getByRole('heading', { level: 1 })).toBeVisible();
  const darkCtas = page.locator('[data-testid="window-installers-demo-cta"]');
  ...
  await expect(page.getByRole('heading', { level: 1 })).toBeVisible();
  await expect(
    page.locator('[data-testid="window-installers-demo-cta"]'),
  ).toHaveCount(3);
});
...
test('bottom strategy-call CTA fires the window_installers_book_call analytics event', async ({
  page,
}) => {
  await gotoWindowInstallers(page);
  ...
  const bottomCta = page
    .locator('[data-testid="window-installers-footer-cta"]')
    .first();
  ...
  expect(
    labels.some((l) =>
      /window_installers_book_call.*window_installers_footer/.test(l),
    ),
    `expected an event_label like "window_installers_book_call - window_installers_footer", got: ${labels.join(' | ')}`,
  ).toBe(true);
});

// Standard Box 11: no first-party /api/ call fires when rendering the page,
// and the /ai-for-window-installers URL appears in the generated sitemap
...
test('makes no first-party /api/ call and appears in sitemap.xml', async ({
  ...
  expect(
    apiCalls,
    `the window-installers landing page should make no first-party /api/ call:\n${apiCalls.join('\n')}`,
  ).toEqual([]);
  ...
  expect(xml).toContain('https://digitalcraftai.com/ai-for-window-installers');

  // The route is in the ROUTES allow-list.
  expect(STATIC_ROUTES.has('/ai-for-window-installers')).toBe(true);
});
