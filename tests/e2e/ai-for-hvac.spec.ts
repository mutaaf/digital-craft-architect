...
// Ticket 0020 - AI-for-HVAC long-tail landing page funneling into the
// existing home-services demos. Each test maps 1:1 to an acceptance-criteria
// box on the ticket. Modeled directly on tests/e2e/ai-for-plumbers.spec.ts
// (ticket 0017).
//
// IMPORTANT: /ai-for-hvac is NOT in the index.html SEO Pilot `pages` table
...
async function gotoHvac(page: Page): Promise<string[]> {
  const errors: string[] = [];
  page.on('pageerror', (e) => {
    const msg = `pageerror: ${e.message}`;
    if (!isIgnorable(msg)) errors.push(msg);
  });
  const response = await page.goto('/ai-for-hvac', { waitUntil: 'domcontentloaded' });
  expect(response, 'no response for /ai-for-hvac').not.toBeNull();
  expect(
    response!.status(),
    `/ai-for-hvac returned ${response!.status()}`,
  ).toBeLessThan(400);
  await expect
    .poll(
...
// Box 1: a new /ai-for-hvac route renders a page whose Helmet-emitted <title>,
// H1, and meta description all explicitly name "HVAC" or "HVAC contractors,"
// and whose body lists at least three HVAC-specific pain points (after-hours
// emergency calls, maintenance-plan and equipment-swap quoting, post-service
// review timing) with defensible language and no invented client names.
test('renders HVAC-specific H1, Helmet meta description, and three pain points', async ({
  page,
}) => {
  const errors = await gotoHvac(page);

  // <title> assertion is delegated to the Helmet-managed head: the page
  // commits to an HVAC-named title in source, but the runtime <title>
  ...
  await expect
    ...
    .toEqual(expect.arrayContaining([expect.stringMatching(/hvac/i)]));

  const descriptions = await page
    .locator('head meta[name="description"]')
    .evaluateAll((nodes) => nodes.map((n) => n.getAttribute('content') ?? ''));
  const helmetDescription = descriptions[descriptions.length - 1];
  expect(helmetDescription.toLowerCase()).toMatch(/hvac/);
  expect(helmetDescription.length).toBeGreaterThan(40);

  // Three HVAC-specific pain point themes named on the page.
  const body = (await page.locator('body').innerText()).toLowerCase();
  expect(body).toMatch(/after.?hours/);
  expect(body).toMatch(/maintenance|equipment/);
...
// Box 3: the page emits one BreadcrumbList JSON-LD block (Home -> AI for HVAC)
// matching the pattern in src/pages/AiForPlumbers.tsx, and one Service JSON-LD
// whose serviceType mentions HVAC; both parse as valid JSON.
test('emits BreadcrumbList and Service JSON-LD blocks that parse and name HVAC', async ({
  page,
}) => {
  const errors = await gotoHvac(page);
  ...
  expect((items[1].name ?? '').toLowerCase()).toMatch(/hvac/);
  expect(items[1].item).toBe('https://digitalcraftai.com/ai-for-hvac');
  ...
  // shared head). The page's Service block carries the /ai-for-hvac URL
  // OR a serviceType naming HVAC.
  const ours = services.filter(
    (s) =>
      (s.data.url ?? '').includes('/ai-for-hvac') ||
      /hvac/i.test(s.data.serviceType ?? ''),
  );
  expect(ours.length, 'at least one Service block must name HVAC').toBeGreaterThan(0);
  expect(/hvac/i.test(ours[0].data.serviceType ?? '')).toBe(true);
...
// /ai-for-hvac URL appears in the generated sitemap because it is registered
// as a top-level <Route path=...> in src/App.tsx.
test('renders Navbar + Footer, has Helmet canonical, and appears in sitemap.xml', async ({
  ...
  await expect
    ...
    .toEqual(
      expect.arrayContaining([expect.stringContaining('/ai-for-hvac')]),
    );
  ...
  await expect
    ...
    .toEqual(
      expect.arrayContaining([expect.stringMatching(/hvac/i)]),
    );
  ...
  expect(xml).toContain('https://digitalcraftai.com/ai-for-hvac');
...
// each demo CTA with a location label that includes "hvac" so the page's
// funnel is measurable in GA.
test('renders in light/dark on mobile, no em-dash, fires trackCTAClick with hvac label', async ({
  page,
}) => {
  const errors = await gotoHvac(page);
  ...
  // demo CTA click pushes an event with a "hvac" location label.
  await page.evaluate(() => {
  ...
  expect(
    labels.some((l) => /hvac/i.test(l)),
    `expected at least one cta_click event_label to contain "hvac", got: ${labels.join(' | ')}`,
  ).toBe(true);
...
test('makes no first-party /api/ call when rendering and no new hostname appears', async ({
  ...
  expect(
    apiCalls,
    `the HVAC landing page should make no first-party /api/ call:\n${apiCalls.join('\n')}`,
  ).toEqual([]);
  expect(errors).toEqual([]);
});
