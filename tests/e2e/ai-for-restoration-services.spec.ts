...
// Ticket 0072 - AI-for-restoration-services long-tail landing page funneling
// into the existing home-services demos (lead-responder, voice-followup,
// estimate). Each test maps 1:1 to an acceptance-criteria box on the ticket.
// Modeled directly on tests/e2e/ai-for-pool-service.spec.ts (ticket 0058, the
// closest peer because both are residential service trades with a 24/7
// emergency funnel where the after-hours call IS the business). NOT modeled
// on the property-managers spec because that page routes into
// /realestate/demo/* (the wrong family for restoration and remediation).
//
// IMPORTANT: /ai-for-restoration-services is NOT in the index.html SEO Pilot
...
// /ai-for-restoration-services-scoped BreadcrumbList block cannot collide.
// demo-breadcrumbs.spec.ts iterates a hard-coded STARTER_ROUTES list that
// does NOT include /ai-for-restoration-services, so it is unaffected.
...
async function gotoRestorationServices(page: Page): Promise<string[]> {
  const errors: string[] = [];
  page.on('pageerror', (e) => {
    const msg = `pageerror: ${e.message}`;
    if (!isIgnorable(msg)) errors.push(msg);
  });
  const response = await page.goto('/ai-for-restoration-services', { waitUntil: 'domcontentloaded' });
  expect(response, 'no response for /ai-for-restoration-services').not.toBeNull();
  expect(
    response!.status(),
    `/ai-for-restoration-services returned ${response!.status()}`,
  ).toBeLessThan(400);
  await expect
    .poll(
...
// Box 1: a new /ai-for-restoration-services route renders a page whose
// Helmet-emitted description and visible H1 explicitly name "restoration,"
// and whose body lists three trade-specific pain themes (after-hours water
// and fire emergency calls hitting voicemail, insurance scope questions a
// dispatcher cannot answer at 2am, review-request timing that slips the day
// a crew finishes a mitigation job) with defensible language and no
// invented client names, dollar figures, or efficacy percentages.
test('renders restoration-services H1, Helmet meta description, and three pain points', async ({
  page,
}) => {
  const errors = await gotoRestorationServices(page);
  ...
  // "Restoration" (case-insensitive) covers both "Restoration Companies"
  // and "Restoration Business" copy variants.
  const h1 = page.getByRole('heading', { level: 1 });
  await expect(h1).toBeVisible();
  const h1Text = ((await h1.textContent()) ?? '').toLowerCase();
  expect(h1Text).toMatch(/restoration/);
  ...
  await expect
    ...
    .toEqual(expect.arrayContaining([expect.stringMatching(/restoration/i)]));

  const descriptions = await page
    .locator('head meta[name="description"]')
    .evaluateAll((nodes) => nodes.map((n) => n.getAttribute('content') ?? ''));
  const helmetDescription = descriptions[descriptions.length - 1];
  expect(helmetDescription.toLowerCase()).toMatch(/restoration/);
  expect(helmetDescription.length).toBeGreaterThan(40);

  // Three trade-specific pain point themes named on the page.
  const body = (await page.locator('body').innerText()).toLowerCase();
  expect(body).toMatch(/water|fire|emergency|voicemail|after[-\s]?hours/);
  expect(body).toMatch(/insurance|scope|carrier|dispatcher/);
  expect(body).toMatch(/review|timing|mitigation|crew/);
...
test('surfaces three demo CTA links pointing at the existing home-services demos', async ({
  ...
  // All three target routes are real static routes (smoke gate covers their
  ...
  // data-testid="restoration-demo-cta" so the spec can locate them without
  ...
  const ctas = page.locator('[data-testid="restoration-demo-cta"]');
...
// (Home -> AI for Restoration Services) that parses as valid JSON. Per the
// 2026-05-30 second-@type lesson, the predicate is per-URL scoped
// (gotoRestorationServices was called first), so this toHaveLength(1) cannot
// collide with a sibling page's BreadcrumbList block. The ten existing
// trade pages emit BreadcrumbList only - the eleventh follows that
// convention.
test('emits BreadcrumbList JSON-LD block that parses and names AI for Restoration Services', async ({
  page,
}) => {
  const errors = await gotoRestorationServices(page);
  ...
  // Second ListItem must explicitly name "AI for Restoration Services" per
  // ticket acceptance box 2 and match the H1 substring pattern.
  expect(items[1].name ?? '').toBe('AI for Restoration Services');
  expect((items[1].name ?? '').toLowerCase()).toMatch(/restoration/);
  expect(items[1].item).toBe('https://digitalcraftai.com/ai-for-restoration-services');
...
// title/description/canonical/OG, and the /ai-for-restoration-services URL
...
test('renders Navbar + Footer, has Helmet canonical, and appears in sitemap.xml', async ({
  ...
  // the restoration-services route.
  await expect
    .poll(
      async () =>
        page
    ...
    .toEqual(
      expect.arrayContaining([expect.stringContaining('/ai-for-restoration-services')]),
    );
  ...
  await expect
    ...
    .toEqual(
      expect.arrayContaining([expect.stringMatching(/restoration/i)]),
    );
  ...
  expect(xml).toContain('https://digitalcraftai.com/ai-for-restoration-services');

  // The new route is in the ROUTES allow-list (canonical src/data/routes.ts
  // per the 2026-06-07 lesson) - the spec re-exports it.
  expect(STATIC_ROUTES.has('/ai-for-restoration-services')).toBe(true);
...
// fired on each demo CTA with a location label that includes "restoration"
// so the page's funnel is measurable in GA independently of the ten other
// trade pages.
test('renders in light/dark on mobile, no em-dash, fires trackCTAClick with restoration label', async ({
  page,
}) => {
  const errors = await gotoRestorationServices(page);
  ...
  // demo CTA click pushes an event with a restoration location label.
  await page.evaluate(() => {
  ...
  expect(
    labels.some((l) => /restoration/i.test(l)),
    `expected at least one cta_click event_label to name restoration, got: ${labels.join(' | ')}`,
  ).toBe(true);
...
test('makes no first-party /api/ call when rendering and no new hostname appears', async ({
  ...
  expect(
    apiCalls,
    `the restoration-services landing page should make no first-party /api/ call:\n${apiCalls.join('\n')}`,
  ).toEqual([]);
  expect(errors).toEqual([]);
});
