...
// Ticket 0065 - Comparison page "Digital Craft vs Follow Up Boss" for
// real-estate CRM switchers. Each test maps 1:1 to an acceptance-criteria
// box on the ticket. Modeled on tests/e2e/compare-thumbtack.spec.ts
// (ticket 0049, the most recent peer that also breadcrumbs through the
// /compare hub).
//
...
// code. Every "exactly one" predicate is URL-scoped to a different route
// (/compare/{jobber,servicetitan,podium,housecallpro,buildertrend,
// thumbtack,angi}, /compare, /changelog, /trust, /texas, /quiz, /glossary,
// per-case-study). None assert "exactly one of either @type site-wide", so
// a new /compare/followupboss-scoped pair does NOT collide. Recorded in
// the ticket Implementation log.
//
// Per the 2026-05-25 SEO Pilot lesson, /compare/followupboss is NOT in
// the index.html SEO Pilot `pages` table; the spec asserts the LAST
...
async function gotoFollowUpBoss(page: Page): Promise<string[]> {
  const errors: string[] = [];
  page.on('pageerror', (e) => {
    const msg = `pageerror: ${e.message}`;
    if (!isIgnorable(msg)) errors.push(msg);
  });
  const response = await page.goto('/compare/followupboss', { waitUntil: 'domcontentloaded' });
  expect(response, 'no response for /compare/followupboss').not.toBeNull();
  expect(
    response!.status(),
    `/compare/followupboss returned ${response!.status()}`,
  ).toBeLessThan(400);
...
// Box 1: /compare/followupboss renders a page whose Helmet meta
// description names BOTH Follow Up Boss and Digital Craft, and whose H1
// names both. The page positions Follow Up Boss as a real-estate CRM and
// Digital Craft as the AI agent layer that answers the leads a CRM
// stores. The page contains a four-row comparison table for the
// dimensions named in the ticket (Lead capture, First response, Property
// analysis, Negotiation).
test('renders Helmet meta + H1 naming both Follow Up Boss and Digital Craft', async ({ page }) => {
  const errors = await gotoFollowUpBoss(page);

  const h1 = page.getByRole('heading', { level: 1 });
  await expect(h1).toBeVisible();
  const h1Text = ((await h1.textContent()) ?? '').toLowerCase();
  expect(h1Text).toMatch(/follow up boss/);
  ...
  await expect
    ...
    .toEqual(expect.arrayContaining([expect.stringMatching(/follow up boss/i)]));

  const descriptions = await page
    .locator('head meta[name="description"]')
    .evaluateAll((nodes) => nodes.map((n) => n.getAttribute('content') ?? ''));
  const helmetDescription = descriptions[descriptions.length - 1];
  expect(helmetDescription.toLowerCase()).toMatch(/follow up boss/);
  ...
  await expect
    ...
    .toEqual(
      expect.arrayContaining([expect.stringContaining('/compare/followupboss')]),
    );
...
// capture, First response, Property analysis, Negotiation) AND
// defensibly positions Follow Up Boss as the storage/pipeline CRM rather
// than trashing it.
test('comparison table covers the four Follow Up Boss-specific dimensions', async ({ page }) => {
  const errors = await gotoFollowUpBoss(page);
  ...
  expect(body, 'page must name Property analysis dimension').toMatch(/property analysis/);
  expect(body, 'page must name Negotiation dimension').toMatch(/negotiation/);

  // Honest positioning: page must acknowledge Follow Up Boss is a
  // real-estate CRM that stores/nurtures leads, not trash it.
  expect(body, 'page must position Follow Up Boss as a CRM').toMatch(
    /crm|nurture|pipeline|action plan/,
  );
...
test('emits BreadcrumbList whose middle item is Compare linking to /compare', async ({ page }) => {
  ...
  expect(
    breadcrumbs,
    'exactly one BreadcrumbList block expected on /compare/followupboss',
  ).toHaveLength(1);
  ...
  expect(items[2].name).toMatch(/Follow Up Boss/);
  expect(items[2].item).toBe(`${ORIGIN}/compare/followupboss`);
...
test('emits WebPage JSON-LD with name, description, and isPartOf WebSite', async ({ page }) => {
  ...
  const ours = webpages.filter((w) =>
    (w.data.name ?? '').toLowerCase().includes('follow up boss'),
  );
  expect(
    ours.length,
    'exactly one WebPage block naming Follow Up Boss expected on /compare/followupboss',
  ).toBe(1);

  const wp = ours[0].data;
  expect(typeof wp.name).toBe('string');
  expect((wp.name ?? '').toLowerCase()).toMatch(/follow up boss/);
...
// comparefollowupboss_ event_label.
test('three demo CTAs route to realestate demos; strategy CTA opens calendly safely', async ({
  page,
}) => {
  const errors = await gotoFollowUpBoss(page);

  const demoCTAs = page.locator('[data-testid="comparefollowupboss-demo-cta"]');
  ...
  // one event_label contains the "comparefollowupboss_" prefix.
  await page.evaluate(() => {
  ...
  expect(
    labels.some((l) => /comparefollowupboss_/i.test(l)),
    `expected at least one cta_click event_label to contain "comparefollowupboss_", got: ${labels.join(' | ')}`,
  ).toBe(true);
...
test('makes no first-party /api/ call when rendering', async ({ page }) => {
  ...
  expect(
    apiCalls,
    `the Follow Up Boss comparison page should make no first-party /api/ call:\n${apiCalls.join('\n')}`,
  ).toEqual([]);
  expect(errors).toEqual([]);
});
...
test('renders Navbar + Footer and appears in sitemap.xml', async ({ page }) => {
  ...
  expect(xml).toContain('https://digitalcraftai.com/compare/followupboss');

  expect(errors).toEqual([]);
});

// Box 9: COMPARE_ENTRIES has an entry with id "followupboss" whose path
// is /compare/followupboss and whose tagline contains no em-dash.
test('COMPARE_ENTRIES contains the followupboss entry', () => {
  const entry = COMPARE_ENTRIES.find((e) => e.id === 'followupboss');
  expect(entry, 'COMPARE_ENTRIES must contain a followupboss entry').toBeDefined();
  expect(entry?.tool).toBe('Follow Up Boss');
  expect(entry?.path).toBe('/compare/followupboss');
