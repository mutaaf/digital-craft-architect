...
// Ticket 0041 - AI-for-landscapers long-tail landing page funneling into the
...
// (ticket 0034), the closest peer.
//
// IMPORTANT: /ai-for-landscapers is NOT in the index.html SEO Pilot `pages`
...
async function gotoLandscapers(page: Page): Promise<string[]> {
  const errors: string[] = [];
  page.on('pageerror', (e) => {
    const msg = `pageerror: ${e.message}`;
    if (!isIgnorable(msg)) errors.push(msg);
  });
  const response = await page.goto('/ai-for-landscapers', { waitUntil: 'domcontentloaded' });
  expect(response, 'no response for /ai-for-landscapers').not.toBeNull();
  expect(
    response!.status(),
    `/ai-for-landscapers returned ${response!.status()}`,
  ).toBeLessThan(400);
  await expect
    .poll(
...
// Box 1: a new /ai-for-landscapers route renders a page whose Helmet-emitted
// description, H1, and meta description all explicitly name "landscaping"
// or "landscapers," and whose body lists three trade-specific pain themes
// (no-shows on spring-cleanup quotes, slow maintenance-contract bid
// turnaround, missed review windows after a finished install) with
// defensible language and no invented client names, dollar figures, or
// efficacy percentages.
test('renders landscaper-specific H1, Helmet meta description, and three pain points', async ({
  page,
}) => {
  const errors = await gotoLandscapers(page);

  // <title> assertion is delegated to the Helmet-managed head: the page
  // commits to a landscaping-named title in source, but the runtime <title>
  ...
  // H1: the visible page heading must also name the trade. Per ticket box 5
  // the substring "Landscap" (case-insensitive) covers both "Landscaper" and
  // "Landscaping" so the assertion is stable to copy-edits within the family.
  const h1 = page.getByRole('heading', { level: 1 });
  await expect(h1).toBeVisible();
  const h1Text = ((await h1.textContent()) ?? '').toLowerCase();
  expect(h1Text).toMatch(/landscap/);
  ...
  await expect
    ...
    .toEqual(expect.arrayContaining([expect.stringMatching(/landscap/i)]));

  const descriptions = await page
    .locator('head meta[name="description"]')
    .evaluateAll((nodes) => nodes.map((n) => n.getAttribute('content') ?? ''));
  const helmetDescription = descriptions[descriptions.length - 1];
  expect(helmetDescription.toLowerCase()).toMatch(/landscap/);
  expect(helmetDescription.length).toBeGreaterThan(40);

  // Three trade-specific pain point themes named on the page.
  const body = (await page.locator('body').innerText()).toLowerCase();
  expect(body).toMatch(/spring[-\s]?cleanup|cleanup/);
  expect(body).toMatch(/maintenance[-\s]?contract|maintenance/);
...
test('surfaces two demo CTA links pointing at the existing home-services demos', async ({
  ...
  // Per box 5 the two demo-card CTAs also carry data-testid="landscapers-demo-cta"
  // so the spec can locate them without relying on body copy. Each one's href
  // resolves to a /homeservices/demo/* path registered in ROUTES.
  const ctas = page.locator('[data-testid="landscapers-demo-cta"]');
  await expect(ctas).toHaveCount(2);
  const ctaHrefs = await ctas.evaluateAll((nodes) =>
    nodes.map((n) => (n as HTMLAnchorElement).getAttribute('href') ?? ''),
  );
  for (const href of ctaHrefs) {
    expect(href.startsWith('/homeservices/demo/')).toBe(true);
...
// Landscapers) and one Service JSON-LD whose serviceType mentions
// landscape work; both parse as valid JSON.
test('emits BreadcrumbList and Service JSON-LD blocks that parse and name landscapers', async ({
  page,
}) => {
  const errors = await gotoLandscapers(page);
  ...
  expect((items[1].name ?? '').toLowerCase()).toMatch(/landscap/);
  expect(items[1].item).toBe('https://digitalcraftai.com/ai-for-landscapers');
  ...
  // shared head). The page's Service block carries the /ai-for-landscapers
  // URL OR a serviceType naming landscape work.
  const ours = services.filter(
    (s) =>
      (s.data.url ?? '').includes('/ai-for-landscapers') ||
      /landscap/i.test(s.data.serviceType ?? ''),
  );
  expect(ours.length, 'at least one Service block must name landscape work').toBeGreaterThan(0);
  expect(/landscap/i.test(ours[0].data.serviceType ?? '')).toBe(true);
...
// /ai-for-landscapers URL appears in the generated sitemap because it is
...
test('renders Navbar + Footer, has Helmet canonical, and appears in sitemap.xml', async ({
  ...
  await expect
    ...
    .toEqual(
      expect.arrayContaining([expect.stringContaining('/ai-for-landscapers')]),
    );
  ...
  await expect
    ...
    .toEqual(
      expect.arrayContaining([expect.stringMatching(/landscap/i)]),
    );
  ...
  expect(xml).toContain('https://digitalcraftai.com/ai-for-landscapers');
...
// fired on each demo CTA with a location label that includes "landscapers"
// so the page's funnel is measurable in GA.
test('renders in light/dark on mobile, no em-dash, fires trackCTAClick with landscapers label', async ({
  page,
}) => {
  const errors = await gotoLandscapers(page);
  ...
  // demo CTA click pushes an event with a landscapers location label.
  await page.evaluate(() => {
  ...
  expect(
    labels.some((l) => /landscapers/i.test(l)),
    `expected at least one cta_click event_label to name landscapers, got: ${labels.join(' | ')}`,
  ).toBe(true);
...
test('makes no first-party /api/ call when rendering and no new hostname appears', async ({
  ...
  expect(
    apiCalls,
    `the landscapers landing page should make no first-party /api/ call:\n${apiCalls.join('\n')}`,
  ).toEqual([]);
  expect(errors).toEqual([]);
});
