...
// Ticket 0096 - AI-for-fencing-contractors long-tail landing page funneling
...
// ticket. Structurally modeled on tests/e2e/ai-for-tree-services.spec.ts
// (ticket 0092, the freshest peer in the home-services trade-page family)
...
// /ai-for-fencing-contractors-scoped Service, BreadcrumbList, or FAQPage
...
// lesson, `gotoFencingContractors` waits for the RouteFallback spinner to
...
async function gotoFencingContractors(page: Page): Promise<void> {
  const response = await page.goto('/ai-for-fencing-contractors', {
    waitUntil: 'domcontentloaded',
  });
  expect(response, 'no response for /ai-for-fencing-contractors').not.toBeNull();
  // Box: GET /ai-for-fencing-contractors returns 200.
  expect(
    response!.status(),
    `/ai-for-fencing-contractors returned ${response!.status()}`,
  ).toBeLessThan(400);
...
// Box: page renders an H1 containing "Fencing Contractors"
// (case-insensitive). Also covers the ticket's "hero H1 'AI for Fencing
// Contractors'" acceptance box.
test('renders H1 naming Fencing Contractors and a defensible subtitle', async ({
  page,
}) => {
  await gotoFencingContractors(page);

  const h1 = page.getByRole('heading', { level: 1 });
  await expect(h1).toBeVisible();
  const h1Text = ((await h1.textContent()) ?? '').toLowerCase();
  expect(h1Text).toMatch(/fencing contractors/);

  // Helmet appends its own meta[name="description"]; poll the head to catch
  // the description Helmet writes after hydrate.
  await expect
    .poll(
      async () =>
        page
    ...
    .toEqual(
      expect.arrayContaining([expect.stringMatching(/fencing contractor/i)]),
    );

  // Subtitle acceptance: the page body names the pain of after-hours
  // inbound quote requests going to voicemail, long-tail linear-footage
  // and material-choice triage, and next-morning appointment follow-ups.
  const body = (await page.locator('body').innerText()).toLowerCase();
  expect(body).toMatch(/voicemail|after.?hours|saturday|install/);
  expect(body).toMatch(
    /linear.?foot|cedar|vinyl|chain.?link|ornamental|aluminum|picket|welded|privacy/,
  );
  expect(body).toMatch(/measure|appointment|crew|arrival|follow.?up/);
});

// Box: page renders exactly three demo-card CTAs with hrefs
...
test('surfaces three demo CTA links pointing at the existing home-services demos', async ({
  ...
  expect(hrefs, 'page must link to /homeservices/demo/estimate').toContain(
  ...
  // Each demo-card CTA carries data-testid="fencing-contractors-demo-cta" so
  // the spec can find the exact three cards without depending on body copy.
  const ctas = page.locator('[data-testid="fencing-contractors-demo-cta"]');
...
// Box: exactly one Service JSON-LD block whose `name` field contains
// "Fencing Contractors". Per the 2026-05-30 second-@type lesson, this
// predicate is URL-scoped (gotoFencingContractors was called first) so it
// cannot collide with any predecessor trade-page's exactly-one-Service
// assertion.
test('emits exactly one Service JSON-LD block naming Fencing Contractors', async ({
  page,
}) => {
  await gotoFencingContractors(page);
  ...
  expect(svc.name ?? '').toMatch(/fencing contractors/i);
  expect(svc.url).toBe('https://digitalcraftai.com/ai-for-fencing-contractors');
...
// Box: exactly one BreadcrumbList JSON-LD block with two itemListElement
// entries whose names are "Home" and "AI for Fencing Contractors".
// URL-scoped per the 2026-05-30 lesson.
test('emits exactly one BreadcrumbList block whose second item is AI for Fencing Contractors', async ({
  page,
}) => {
  await gotoFencingContractors(page);
  const blocks = await readJsonLdBlocks(page);
  ...
  expect(
    breadcrumbs,
    'exactly one BreadcrumbList block expected',
  ).toHaveLength(1);
  ...
  expect(items[1].name ?? '').toBe('AI for Fencing Contractors');
  expect(items[1].item).toBe(
    'https://digitalcraftai.com/ai-for-fencing-contractors',
  );
...
// Box: exactly one FAQPage JSON-LD block whose `mainEntity` array length
...
test('emits exactly one FAQPage block mirroring the visible FAQ cards', async ({
  ...
  // answer appears verbatim in the FAQPage block.
...
// Box: no em-dash (U+2014) anywhere in the visible body copy, and none in
// any JSON-LD block this page owns. Filter to the page's own @types per
// the 2026-09-08 lesson so we do not trip the shipped homepage
...
// Box: the page renders cleanly in both light and dark mode. Toggle the
// html.dark class the way the ticket 0092 spec does and re-check that the
...
test('renders in light and dark mode with the same demo-card CTAs visible', async ({
  ...
  await expect(page.getByRole('heading', { level: 1 })).toBeVisible();
  await expect(
    page.locator('[data-testid="fencing-contractors-demo-cta"]'),
  ).toHaveCount(3);
});

// Box: the "Book a 15-minute strategy call" CTA at the bottom fires
...
test('bottom strategy-call CTA fires the fencing_contractors_book_call analytics event', async ({
  page,
}) => {
  await gotoFencingContractors(page);
  ...
  const bottomCta = page
    .locator('[data-testid="fencing-contractors-footer-cta"]')
    .first();
  ...
  expect(
    labels.some((l) =>
      /fencing_contractors_book_call.*fencing_contractors_footer/.test(l),
    ),
    `expected an event_label like "fencing_contractors_book_call - fencing_contractors_footer", got: ${labels.join(' | ')}`,
  ).toBe(true);
});

// Standard box: no first-party /api/ call fires when rendering the page,
// and the /ai-for-fencing-contractors URL appears in the generated sitemap
...
test('makes no first-party /api/ call and appears in sitemap.xml', async ({
  ...
  expect(
    apiCalls,
    `the fencing-contractors landing page should make no first-party /api/ call:\n${apiCalls.join('\n')}`,
  ).toEqual([]);
  ...
  expect(xml).toContain('https://digitalcraftai.com/ai-for-fencing-contractors');

  // The route is in the ROUTES allow-list.
  expect(STATIC_ROUTES.has('/ai-for-fencing-contractors')).toBe(true);
});
