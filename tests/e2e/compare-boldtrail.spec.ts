...
// Ticket 0099 - Comparison page "Digital Craft vs BoldTrail" for real-
// estate CRM lead-generation switchers. Each test maps 1:1 to an
// acceptance-criteria box on the ticket. Modeled on
// tests/e2e/compare-kvcore.spec.ts (ticket 0086, the direct real-estate-
// vertical peer at similar file size AND hub-aware breadcrumb shape).
//
...
// housecallpro,buildertrend,thumbtack,angi,followupboss,jobtread,kvcore},
// /compare, /changelog, /trust, /texas, /quiz, /glossary, per-case-study,
// blog collection). None assert "exactly one of either @type site-wide",
// so a sixteenth /compare/boldtrail-scoped pair does NOT collide.
// Recorded in the ticket Implementation log.
//
// Per the 2026-05-25 SEO Pilot lesson, /compare/boldtrail is NOT in the
...
// em-dash and must not be flagged by the /compare/boldtrail spec.
//
// Per the 2026-09-05 route-code-splitting + 2026-09-10 mount-signal
// lessons, gotoBoldTrail waits for the H1 to be visible instead of
...
async function gotoBoldTrail(page: Page): Promise<string[]> {
  const errors: string[] = [];
  page.on('pageerror', (e) => {
    const msg = `pageerror: ${e.message}`;
    if (!isIgnorable(msg)) errors.push(msg);
  });
  const response = await page.goto('/compare/boldtrail', { waitUntil: 'domcontentloaded' });
  expect(response, 'no response for /compare/boldtrail').not.toBeNull();
  expect(
    response!.status(),
    `/compare/boldtrail returned ${response!.status()}`,
  ).toBeLessThan(400);
  // Per the 2026-09-05 route-code-splitting + 2026-09-10 mount-signal
...
// Box 1: /compare/boldtrail renders a page whose Helmet meta description
// names BOTH BoldTrail and Digital Craft, and whose H1 names both. The
// page positions BoldTrail as Inside Real Estate's next-generation
// residential-real-estate operating system bundling IDX websites,
// squeeze pages, paid-lead capture, and CRM and Digital Craft as the AI
// agent layer that answers the leads a real-estate CRM stores.
test('renders Helmet meta + H1 naming both BoldTrail and Digital Craft', async ({ page }) => {
  const errors = await gotoBoldTrail(page);

  const h1 = page.getByRole('heading', { level: 1 });
  await expect(h1).toBeVisible();
  const h1Text = ((await h1.textContent()) ?? '').toLowerCase();
  expect(h1Text).toMatch(/boldtrail/);
  ...
  await expect
    ...
    .toEqual(expect.arrayContaining([expect.stringMatching(/boldtrail/i)]));

  const descriptions = await page
    .locator('head meta[name="description"]')
    .evaluateAll((nodes) => nodes.map((n) => n.getAttribute('content') ?? ''));
  const helmetDescription = descriptions[descriptions.length - 1];
  expect(helmetDescription.toLowerCase()).toMatch(/boldtrail/);
  ...
  await expect
    ...
    .toEqual(
      expect.arrayContaining([expect.stringContaining('/compare/boldtrail')]),
    );
...
// defensibly positions BoldTrail as Inside Real Estate's IDX-plus-CRM-
// plus-paid-lead operating system rather than trashing it.
test('comparison table covers the four BoldTrail-specific dimensions', async ({ page }) => {
  const errors = await gotoBoldTrail(page);

  const rows = page.locator('[data-testid="boldtrail-comparison-row"]');
  ...
  // Honest positioning: page must acknowledge BoldTrail bundles IDX +
  // squeeze pages + paid-lead capture + CRM (its actual real-estate-
  // operating-system value prop under Inside Real Estate), not trash it.
  expect(body, 'page must position BoldTrail as a real-estate operating system').toMatch(
    /idx|squeeze|smart plan|drip|crm|paid lead/,
  );
  expect(body, 'page must reference the Inside Real Estate parent brand').toMatch(
    /inside real estate/,
  );
...
test('emits BreadcrumbList whose middle item is Compare linking to /compare', async ({ page }) => {
  ...
  expect(
    breadcrumbs,
    'exactly one BreadcrumbList block expected on /compare/boldtrail',
  ).toHaveLength(1);
  ...
  expect(items[2].name).toMatch(/BoldTrail/i);
  expect(items[2].item).toBe(`${ORIGIN}/compare/boldtrail`);
...
test('emits WebPage JSON-LD with name, description, and isPartOf WebSite', async ({ page }) => {
  ...
  const ours = webpages.filter((w) =>
    (w.data.name ?? '').toLowerCase().includes('boldtrail'),
  );
  expect(
    ours.length,
    'exactly one WebPage block naming BoldTrail expected on /compare/boldtrail',
  ).toBe(1);

  const wp = ours[0].data;
  expect(typeof wp.name).toBe('string');
  expect((wp.name ?? '').toLowerCase()).toMatch(/boldtrail/);
...
// compareboldtrail_ event_label.
test('three demo CTAs route to realestate demos; strategy CTA opens calendly safely', async ({
  page,
}) => {
  const errors = await gotoBoldTrail(page);

  const demoCTAs = page.locator('[data-testid="compareboldtrail-demo-cta"]');
  ...
  // one event_label contains the "compareboldtrail_" prefix.
  await page.evaluate(() => {
  ...
  expect(
    labels.some((l) => /compareboldtrail_/i.test(l)),
    `expected at least one cta_click event_label to contain "compareboldtrail_", got: ${labels.join(' | ')}`,
  ).toBe(true);
...
test('makes no first-party /api/ call when rendering', async ({ page }) => {
  ...
  expect(
    apiCalls,
    `the BoldTrail comparison page should make no first-party /api/ call:\n${apiCalls.join('\n')}`,
  ).toEqual([]);
  expect(errors).toEqual([]);
});
...
test('renders Navbar + Footer and appears in sitemap.xml', async ({ page }) => {
  ...
  expect(xml).toContain('https://digitalcraftai.com/compare/boldtrail');

  expect(errors).toEqual([]);
});

// Box 9: COMPARE_ENTRIES has an entry with id "boldtrail" whose path is
// /compare/boldtrail and whose tagline contains no em-dash.
test('COMPARE_ENTRIES contains the boldtrail entry', () => {
  const entry = COMPARE_ENTRIES.find((e) => e.id === 'boldtrail');
  expect(entry, 'COMPARE_ENTRIES must contain a boldtrail entry').toBeDefined();
  expect(entry?.tool).toBe('BoldTrail');
  expect(entry?.path).toBe('/compare/boldtrail');
...
test('sibling-hub regression: /compare card count and ItemList reflect boldtrail entry', async ({
  page,
}) => {
  const errors: string[] = [];
  page.on('pageerror', (e) => {
  ...
  // appending the boldtrail entry).
  const cards = page.locator('[data-testid="compare-hub-card"]');
  await expect(cards.first()).toBeVisible();
  expect(await cards.count()).toBe(COMPARE_ENTRIES.length);

  // (b) a card whose visible text names "BoldTrail" is present.
  const boldtrailCard = page
    .locator('[data-testid="compare-hub-card"]')
    .filter({ hasText: 'BoldTrail' });
  await expect(boldtrailCard).toHaveCount(1);
  const boldtrailCardHref = await boldtrailCard.getAttribute('href');
  expect(boldtrailCardHref).toBe('/compare/boldtrail');
  ...
  const boldtrailItem = (hubList!.data.itemListElement ?? []).find(
    (it) => (it.name ?? '').toLowerCase().includes('boldtrail'),
  );
  expect(
    boldtrailItem,
    'ItemList must include a ListItem naming BoldTrail',
  ).toBeDefined();
  expect(boldtrailItem?.url).toBe(`${ORIGIN}/compare/boldtrail`);

  expect(errors).toEqual([]);
});

// Box 11: /compare.json JSON Feed regression. GET the feed and assert
// items.length equals COMPARE_ENTRIES.length (mirror-source check per
// the ticket 0095 pattern; a mismatch signals the JSON Feed generator
// did not widen when compareEntries.ts was appended).
test('/compare.json JSON Feed widened to include the boldtrail entry', async ({ request }) => {
  const res = await request.get('/compare.json');
  expect(res.status(), '/compare.json must return 200').toBe(200);
  const body = await res.text();
  const feed = JSON.parse(body) as { items: { url: string }[] };
  expect(Array.isArray(feed.items), 'feed.items must be an array').toBe(true);
  expect(feed.items.length).toBe(COMPARE_ENTRIES.length);
  const boldtrailItem = feed.items.find((it) => it.url === `${ORIGIN}/compare/boldtrail`);
  expect(
    boldtrailItem,
    'JSON Feed must include an item for /compare/boldtrail',
  ).toBeDefined();
});
