...
- [ ] STICKY-CTA-LANDING: Add the existing `StickyCTA` component (`src/components/StickyCTA.tsx`) to the main landing pages. Import and render `<StickyCTA />` at the bottom of `src/pages/Index.tsx`, `src/pages/Construction.tsx`, `src/pages/RealEstate.tsx`, and `src/pages/Events.tsx`. It's currently only used in demo hub pages.
