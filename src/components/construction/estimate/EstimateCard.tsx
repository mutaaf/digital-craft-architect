...
const LineItem = ({ label, low, high }: { label: string; low: number; high: number }) => (
  <TableRow>
    ...
    <TableCell className="text-right text-gray-500">{fmt(low)}</TableCell>
    <TableCell className="text-right text-gray-500">{fmt(high)}</TableCell>
  </TableRow>
);
  ...
  return (
    <div className="max-w-2xl mx-auto print:max-w-none">
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-lg overflow-hidden print:shadow-none print:border">
        {/* Header */}
        <div className="bg-gray-900 dark:bg-gray-800 text-white px-8 py-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold">448 Developments</h2>
              <p className="text-gray-300 text-sm mt-1">DFW Construction & Remodeling</p>
            </div>
        ...
        <div className="p-8">
          ...
          <Table>
            <TableBody>
              <TableRow className="text-xs font-semibold text-gray-400 uppercase">
              ...
              {breakdown.extras.map((e) => (
                <TableRow key={e.label}>
                  ...
                  <TableCell className="text-right text-gray-500">{fmt(e.cost)}</TableCell>
                  <TableCell className="text-right text-gray-500">{fmt(e.cost)}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-2">
              ...
              <span className="text-lg font-semibold">Estimated Range</span>
            </div>
