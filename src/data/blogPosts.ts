 ...
 *   date: '2026-04-01',                 // YYYY-MM-DD — ALWAYS use TODAY'S date (run `date +%Y-%m-%d` if unsure). Do NOT copy this example date.
 ...
 *
 * ⚠️ DATE RULE — CRITICAL FOR THE AUTONOMOUS GTM AGENT:
 *   The `date` field MUST be the actual date the post is being added (today).
 *   Run `date +%Y-%m-%d` in the shell and use that exact value. Never copy the
 *   template's example date or reuse a date from another post in this file.
 *   Each post gets a unique date — duplicates skew chronological ordering on
 *   the blog index and look like a content dump to readers and search engines.
 */
