...
## 2026-09-06 - A shipped ticket that pushes the last "matching" row out of a fixed VISIBLE_LIMIT window fails a coincidence-based `> 0` predicate over the derived DOM; widen to the resolver's actual contract, not the window's contents
**Where:** ticket 0067 heal (PR #201), tests/e2e/changelog-itemlist-jsonld.spec.ts Box 7 vs src/pages/Changelog.tsx `resolveDemoPath` (src/pages/Changelog.tsx:109-128) + `VISIBLE_LIMIT = 36` (src/pages/Changelog.tsx:33)
**What went wrong:** Box 7 asserted `page.getByRole('link', { name: /try the demo/i }).count() > 0`. That held only while the visible-36 slice of changelogEntries.ts happened to contain a demos-area entry whose title matched a `KNOWN_PATHS` last-segment token by exact word (0031 "voice"/"estimate", 0029 "voice"). Flipping 0067 to shipped inside this docs-only chore PR regenerated changelogEntries.ts at CI build time with 3 new September rows at the top; that pushed 0031 out of the visible-36 window, leaving the visible demos-area entries (0066, 0062, 0060, 0045, 0040) with `/my`-dashboard-style titles that don't match any `KNOWN_PATHS` segment. `resolveDemoPath` correctly emitted zero links (its docstring explicitly says "the page omits the 'Try the demo' link rather than guess and strand the visitor on a wrong demo"), and the `> 0` assertion red-flagged a docs-only ship for something the resolver was doing exactly right. CI's `retries: 1` did NOT mask it - both attempts failed identically because the DOM state is deterministic once the lazy chunk lands. Diff of the PR touched zero source, only the ticket frontmatter + README row.
**Rule going forward:** When an e2e assertion is a `count > 0` / `toBeGreaterThan(0)` / `toHaveCount(N)` over DOM derived from a VISIBLE_LIMIT-sliced data array, that assertion is coincidence-based whenever the array's ordering is time-sorted and the "matchable" items can drift out of the window as newer items ship. Rewrite it to the underlying resolver's actual contract: iterate every emitted match in the DOM and assert its shape (well-formed href, valid target route, correct testid) - the resolver's positive property holds regardless of window contents, and its "omit rather than guess" negative property is preserved. Same widening family as the 2026-05-30 second-@type-collision lesson (widen the predecessor to admit the new sibling), applied to a data-driven ordering shift instead of a schema type collision. If the "at least one match must be visible" invariant matters for SEO/UX, encode that as a build-time script assertion over `changelogEntries.ts` (analogous to the 2026-05-28 sitemap-lastmod lesson: fail the local gate at generation time, not at Playwright time), not as a Playwright predicate that flakes with data drift.

## 2026-09-05 - Introducing route-level React.lazy breaks e2e helpers that poll only `root.innerHTML.length`
**Where:** PR #181 (`eng/route-code-splitting`) — src/App.tsx RouteFallback, tests/e2e/demo-breadcrumbs.spec.ts gotoRoute (heal commit 08e8fc1)
**What went wrong:** The perf change wrapped every non-landing route in `<Suspense fallback={<RouteFallback />}>`. Most spec `gotoRoute` helpers wait for `document.getElementById('root').innerHTML.length > 500`, then immediately probe the DOM with non-retrying calls (`.allTextContents()`, `.$$eval(...)`, `.count()`). The RouteFallback's markup alone (a min-h-screen spinner div with classes, plus the eager app shell around Toaster/Sonner/Analytics) satisfies that heuristic instantly, so `demo-breadcrumbs.spec.ts:224` read zero crumbs from the still-loading route — deterministically on `/construction/demo/voice-negotiator`, and intermittently on the other lazy routes (2 flakes at :161 and trust-page.spec.ts:156). CI `retries: 1` masked the flakes; the voice-negotiator case failed on retry too because that chunk consistently arrived last.
**Rule going forward:** When introducing route-level lazy loading behind Suspense, DO NOT rely on `root.innerHTML.length > N` as the "page ready" signal in existing e2e helpers — the fallback trips it. Either (a) augment each affected `gotoRoute`/`gotoX` helper with `page.locator('[role="status"][aria-label="Loading"]').waitFor({ state: 'hidden', timeout: 10_000 }).catch(() => {})` after the length poll (safe when the fallback never appears), or (b) give the RouteFallback a unique `data-testid` and wait for its detachment. Also prefer auto-retrying assertions (`await expect(locator).toHaveCount(N)`) over one-shot `.allTextContents()` / `.$$eval` for counts that depend on lazy-loaded DOM. The em-dash test on `/trust` flaking with an extra "DigitalCraft" heading is the same race (dark headings measured while the real page was still rehydrating over the fallback).

## 2026-09-08 - Iterating "every JSON-LD block on the DOM" for an em-dash check trips the shipped homepage Organization block from index.html
**Where:** ticket 0069 heal (PR #209), tests/e2e/subprocessors.spec.ts case 11 vs the ticket 0025 Organization JSON-LD injected via index.html
**What went wrong:** The first version of the /subprocessors em-dash spec looped over ALL `script[type="application/ld+json"]` blocks on the page and asserted `.not.toContain(EM_DASH)` on each `raw` string. That fired on the shipped homepage Organization block (ticket 0025), which ships inline in `index.html` and therefore lands on EVERY route in the SPA. Its `description` field contains a legitimate em-dash (`... small businesses across 12 industries — construction, real estate, ...`) and has been that way since 0025 shipped, so the spec red-flagged a JSON-LD block this PR does not own and cannot edit under the em-dash Hard NO. `retries: 1` did NOT mask it because the block is present deterministically.
**Rule going forward:** When writing a "no em-dash in JSON-LD" case on a NEW page, filter the block list down to the specific `@type`s THIS PAGE actually emits (BreadcrumbList, CollectionPage, ItemList, etc.) BEFORE iterating - never `for (const b of allBlocks)`. The homepage Organization block from `index.html` is site-wide by design and its `description` is legitimate historical copy; any per-page em-dash assertion must scope itself to blocks the page owns. Same "widen to the owner's contract" family as the 2026-09-06 VISIBLE_LIMIT lesson: assert the invariant this page can control, not the union of every block the DOM happens to contain.

## 2026-09-08 - A sibling-hub regression case that SPA-navigates between hubs and asserts each hub's CollectionPage / ItemList must key its poll on the target hub's own `name`, not on `count > 0` JSON-LD scripts
**Where:** ticket 0071 (feat PR #211), tests/e2e/ai-for-hospitality.spec.ts Box 12 sibling-hub regression case
**What went wrong:** Box 12 navigated `/compare` -> `/case-studies` via `page.goto()` and immediately read JSON-LD blocks on the second hub. The reader helper polled until `script[type="application/ld+json"]` count `> 0`, but after the SPA route change Helmet had not yet finished swapping the /compare CollectionPage script tag out - the poll returned instantly with the previous hub's blocks and the /case-studies CollectionPage `name` assertion failed with the /compare hub's name. `retries: 1` did NOT mask it because the swap timing is deterministic-fast on preview. Same failure family as the 2026-09-05 route-code-splitting RouteFallback lesson (a "page ready" heuristic that fires before the target content is actually mounted), but here the stale-DOM window is Helmet head swap during an SPA transition, not Suspense body swap during initial mount.
**Rule going forward:** When a single test navigates between two hubs (SPA route changes without a hard reload) and asserts each hub's CollectionPage / ItemList / SoftwareApplication blocks, do NOT reuse the "count `> 0` JSON-LD scripts" poll that works for a fresh navigation. Extract an `assertHubEmits(page, path, expectedName)` helper that (a) navigates, (b) polls `page.locator('script[type="application/ld+json"]').evaluateAll` until at least one block's `name` equals the target hub's expected `name`, (c) THEN reads and filters blocks by `name` (not by `@type` alone). Same "widen to the resolver's actual contract" family as the 2026-09-06 VISIBLE_LIMIT lesson: assert the specific property this hub controls, not a property the DOM might carry from the previous route.

## 2026-09-10 - A ticket that ships a `useEffect`-on-mount write on lazy-loaded routes needs a mount-completion signal, not just `root.innerHTML.length > 500`
**Where:** ticket 0074 (feat PR #221), tests/e2e/recent-compares-recap.spec.ts write-through and reverse-visit cases vs the RouteFallback race
**What went wrong:** N/A (technique). The write-through and reverse-visit boxes navigate `/compare/<tool>` then read `dca_recent_compares_v1` after arriving at `/my`. The first version of the `gotoPath` helper polled only `document.getElementById('root').innerHTML.length > 500`, which - per the 2026-09-05 lesson - the Suspense RouteFallback already trips before the lazy compare-page chunk actually mounts. The `useEffect(() => recordCompareVisit(...), [])` fires on real mount, so a test that moves to `/my` before mount completes never sees the visit written. Symptom was deterministic on the write-through case (0 rows) and the reverse-visit case (1 row instead of 2).
**Rule going forward:** For any e2e helper that navigates to a lazy-loaded route AND then depends on that route's mount-time side effects (a `useEffect` writing to localStorage, a subscription firing, a Sentry breadcrumb), extend `gotoPath` with a mount-signal wait like `page.locator('h1').first().waitFor({ state: 'visible', timeout: 10_000 }).catch(() => {})` after the innerHTML poll. The `.catch()` keeps the helper compatible with routes that have no `<h1>` where the innerHTML poll is sufficient. Same "RouteFallback trips the readiness heuristic" family as the 2026-09-05 lesson, applied to on-mount write-through rather than DOM-count read-back.

## 2026-09-10 - `suggestNextCompare` must read the raw stored list, not the display-sliced list, so the "all visited" case truly returns null
**Where:** ticket 0074 (feat PR #221), src/utils/recentComparesStore.ts suggestNextCompare vs Box 8 of the new spec
**What went wrong:** N/A (design decision). The initial `suggestNextCompare` implementation called `getRecentCompares()` which slices to `MAX_ENTRIES = 5` on read. When the test seeded all 14 `COMPARE_ENTRIES` paths directly to localStorage (to hit the "every entry has been visited" acceptance box), the slice hid 9 of them and the suggestion returned one of those 9 as "unvisited," failing `toHaveCount(0)` on the chip. In production the recorder truncates to 5, so a real visitor could not hit this state via the UI - but the test seeded raw, which is legitimate.
**Rule going forward:** When a store caps its display list but also exports a derived query ("which entry haven't you visited?"), keep the query reading the RAW parsed list (post-allow-list, pre-slice) and slice only in the public list getter. Extract a private `readAllVisited()` helper that returns the full validated list; expose `getRecentCompares()` as `readAllVisited().slice(0, MAX_ENTRIES)` for display; have `suggestNextCompare()` read `readAllVisited()` directly. Same single-source-of-truth family as the 2026-05-25 mirror-source rule, applied inside a store module rather than across a src/tests boundary.

## 2026-09-12 - Ticket-prose string literals can be placeholders; verify against the real source before locking a mirror-source allow-list
**Where:** ticket 0076 (feat PR #226), src/utils/quizHistoryStore.ts KNOWN_PERSONAS vs src/pages/AIReadinessQuiz.tsx TIERS.label
**What went wrong:** N/A (technique). Ticket 0076's acceptance criteria named the `QuizHistoryEntry.persona` union as `'Just Starting' | 'Getting Smart' | 'AI-Ready'` and said "the same three persona names the existing `quizPersonaStore.ts` uses per ticket 0045; do NOT invent new names." Reading the actual `AIReadinessQuiz.tsx` TIERS table (lines 135-154), the real `.label` strings are `'Getting Started'`, `'Ready for AI'`, `'Advanced - Ready to Scale'` - none of which match the ticket's placeholders. Two other acceptance boxes ALSO tied `KNOWN_PERSONAS` to a byte-identical requirement against `dca_quiz_persona_v1` (the persona snapshot that IS whatever `tier.name` == `TIERS[tier].label` returns at completion time). Implementing the ticket's placeholder union verbatim would have made the allow-list guard drop every real completion at write time (every incoming `tier.label` fails the union check), silently breaking the new store even though tsc, lint, and the standalone spec all pass with seeded data. The mirror-source rule (2026-05-25) resolves the conflict: mirror the REAL source (the code), not the ticket prose that names it.
**Rule going forward:** When a ticket's acceptance criteria embed specific string literals ("the three persona names X, Y, Z", "the four route keys A, B, C, D") AND separately require byte-identical parity with an already-shipped source of truth, grep the named source (`TIERS`, `KNOWN_PATHS`, `ROUTES`, `PERSONAS`, etc.) BEFORE writing tests or code and verify the ticket's list matches. When it doesn't, treat the ticket's list as placeholder prose from the groomer and pin the allow-list to the code's actual strings, then note the deviation in the Implementation log with the file:line of the real source. Same "code beats prose" family as the 2026-05-25 mirror-source rule; this lesson adds: the groomer's inline enumeration is NOT authoritative when the same ticket also demands mirror-parity with a live source.

## 2026-09-26 - Shipping a new static-file extension needs a Vite MIME shim AND (usually) a vercel.json headers rule
**Where:** ticket 0098 (PR #282), tests/e2e/feeds-opml-subscription-index.spec.ts case 1 vs `npm run preview` serving `/feeds.opml` with an empty `Content-Type`
**What went wrong:** Playwright smoke runs against `npm run preview` (Vite's built-in preview, which is `sirv` under the hood). `sirv` reads its MIME table from `mrmime`, which knows `.xml` and `.json` (that is why the ticket 0055 changelog RSS spec and the ticket 0095 compare-JSON spec worked with no server-side wiring) but does NOT know `.opml`. Result: `GET /feeds.opml` returned 200 with `Content-Type: ""`. The `check-links` gate passed (I added `opml` to the asset-extension bypass list) and the local `npm run build` passed (the OPML generator's own invariants only look at the file, not the served response), so this was invisible until the CI smoke suite ran and the very first case of the new spec red-flagged the empty content-type. Vercel production has the same gap (unknown extensions default to `application/octet-stream`), so a vercel.json headers rule is required on top.
**Rule going forward:** Before shipping a new static-file extension (`.opml`, `.ics`, `.gpx`, `.n3`, `.jsonld`, etc.), verify what `npm run preview` returns for a sample file (`curl -sI http://127.0.0.1:4173/<file>`). If the response's `Content-Type` is missing or `application/octet-stream`, ship a tiny Vite plugin in `vite.config.ts` that sets the header for both `configureServer` (dev) and `configurePreviewServer` (preview), AND add a `vercel.json` `headers` entry so production returns the same content-type. Do this in the same PR as the file itself - the local `npm run build` gate does NOT exercise the served content-type, so a missing header only surfaces on CI smoke or on a real fetcher hitting the deployed URL. Same "the local gate catches static shape, not served headers" family as the 2026-05-28 encoded-invariant lesson, applied to HTTP metadata instead of file contents.
