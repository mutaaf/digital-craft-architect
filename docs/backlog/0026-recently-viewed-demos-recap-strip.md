...
- 2026-05-30 - failing e2e spec added at `tests/e2e/recent-demos-recap.spec.ts` with one scenario per acceptance box (first-visit absent, returning-visit shows recap, SPA click navigation, malformed-storage tolerated, dark-mode at 375px, cross-route re-visit moves entry to front, unknown path rejected). Mirrors `tests/e2e/resumable-demo-session.spec.ts` for the `addInitScript` localStorage pre-seed pattern and `tests/e2e/demo-breadcrumbs.spec.ts` for the SPA-marker reload assertion.
- 2026-05-30 - `src/utils/recentDemosStore.ts` (new, under 165 lines) exposes `recordDemoVisit`/`getRecentDemos`/`clearRecentDemos`/`KNOWN_PATHS`, mirroring the parse-safe `try/catch` pattern of `src/pages/construction/lastEstimateStore.ts`. Deviation from engineering notes: kept `KNOWN_PATHS` as a literal `Set` inside the store rather than extracting `DEMO_GROUPS` into a sibling `src/data/demoCatalog.ts` module. The data-extraction path would have added a second file plus rewiring of the `Demos.tsx` import graph; the literal set keeps the diff under 200 lines and is verified by the e2e `unknown path rejected` case plus the `cross-route re-visit` case (which catalog-clicks a real path and asserts it surfaces in the recap).
- 2026-05-30 - `src/pages/Demos.tsx` two minimal edits: import the store, read `getRecentDemos()` once on mount, render a `data-testid="recent-demos-recap"` `<section>` ABOVE the demo groups when `recent.length > 0` (heading "Recently viewed", grid `gap-4 sm:grid-cols-2 lg:grid-cols-3`, each card shows title + muted vertical + "Try it again" link, fires `trackCTAClick('recent_demo', 'demos_hub_recap')` on click), and wrap the existing catalog Link `onClick` to also call `recordDemoVisit(demo.path, demo.title, group.vertical)` alongside the existing `trackCTAClick('open_demo', ...)`. Every new class includes `dark:` variants per the AGENTS.md Hard NO. No em-dash anywhere in rendered copy (Self-Review grep-confirmed).
- 2026-05-30 - merged to main as #93 (commit e7898d7); both gating checks green (`build` 2m9s, `smoke-required` 3s). Status flipped `in-progress` -> `shipped` together with the README index row in `chore/0026-ship-status`.
