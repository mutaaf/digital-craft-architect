...
- 2026-06-19 - failing e2e spec added at `tests/e2e/glossary-breadcrumb-jsonld.spec.ts` (modeled on `tests/e2e/trust-aboutpage-jsonld.spec.ts`); source edit added to `src/pages/Glossary.tsx` (new `BREADCRUMB_SCHEMA` constant + one new `<script type="application/ld+json">` line inside the existing `<Helmet>` block, placed before the DefinedTermSet script). Full local gate green; the new spec + the ticket 0013 spec both pass locally (11/11).
